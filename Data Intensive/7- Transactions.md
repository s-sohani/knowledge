...
در مقایسه روش SSI خوشبینانه هست. خوشبینانه یعنی اینکه بدون بلاک کردن تراکنشی، همه تراکنش ها خوشحال اجرا میشن. اما قبل از کامیت شدن شون دیتابیس چک میکنه مشکلی وجود نداشته باشه . اگر وجود داشت abort میکنه تراکنش رو.

### Decisions based on an outdated premise
دیتابیس از کجا میخاد تشخیص بده که مشکل همروندی پیش اومده یا نه؟
- multi-version concurrency control
- تشخیص تراکنش های نوشتنی که تراکنش های خواندنی قبل از خودشون رو تحت تاثیر قرار میدن



### Detecting stale MVCC reads

روش SSI به شیوه multi-version concurrency control پیاده سازی میشه. وقتی یه تراکنش دیتا رو از یه consistent snapshot میخونه دیگه  نوشتن هایی که بعد از اون اسنپ شات اتفاق افتادن رو ایگنور میکنه.

![[Pasted image 20250203150252.png]]


![[Pasted image 20250203150308.png]]


### Performance of serializable snapshot isolation

پرفورمنس روش SSI
یکی از مزیت هاش نسبت به two-phase locking اینه که یک تراکنش نیازی نداره منتظر تارکنش دیگه ای بمونه. نویسنده ها دیگه خواننده ها را بلاک نمیکنند. و برعکس.
خواننده ها میتونن بدون هیچ نیازی به لاک روی consistent snapshot اجرا بشن.
در مقایسه با اجرای کاملا متوالی تراکنش ها، روش SSI قطعا پرفورمنس بهتری داره.
