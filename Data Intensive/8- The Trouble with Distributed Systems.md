...
الگوریتم ها باید یه طوری طراحی و نوشته بشن که وابسته به جزپیات سیستم زیرساخت نباشن اما با این حال لازمه ما سیستم زیرساخت رو بشناسیم و بدانیم چه خطاهایی از این سیستم انتظار میره.

از نظر timing سیستم ها سه نوع مختلف دارن:
Synchronous model
که در آنها تاخیر نتوورک و توقف ها و clock drift ها نهایتا از یه مقدار مشخصی بیشتر نتونن باشن که خب این شرایط واقعی نیست.
Partially synchronous model
که سیستم مشابه حالت Synchronous هست اما پذیرفته هست که گاهی تاخیرها محدودیت رو رد کنه. این سیستم واقعی تر هست. 
Asynchronous model
این مدل هم که کلا به تایم کاری نداره. کلاک نداریم اینجا. 

علاوه بر timing میشه از منظر node failure به سیستم ها نگاه کرد.
از این نظر سیستم ها انواع مختلف زیر رو دارند:
Crash-stop faults
در این مدل فرض بر این هست که نودها وقتی دچار مشکل میشن که crash کرده باشن. crash کرده باشن یعنی وقتی دچار مشکل شدن دیگه میمیرن.

Crash-recovery faults
در این مدل فرض میکنیم هر نود به هر دلیلی ممکنه crash کنه ولی میتونه دوباره حالش خوب بشه. state اش رو از دست نمیده چون nonvolatile disk storage داره فقط ممکنه قسمتی از وضعیت خودش رو که in-memory بود رو از دست بده.

Byzantine (arbitrary) faults
در این سیستم فرض بر این هست که نودها ممکنه هرکاری که دلشون خواست انجام بدن مثلا بخوان دیگر نودها را فریب بدن.

یه سیستم واقعی معمولا از نوع partially synchronous model و crash-recovery faults هست

### Correctness of an algorithm

صحت هر الگوریتم قابل توصیف هست. برای توصیفش لازم هست ما properties هاشو توصیف کنیم.
مثلا الگوریتم sorting که روی دوتا لیست اعمال میشه خوروجیش باید المان هاش از چپ به راست مرتب شده باشن.

مثلا اگر یه الگوریتم نوشتیم برای تولید fencing token، باید این ویژگی ها رو داشته باشه که توکن های یونیک تولید کنه. توکن ها تولیدی مقدارشون افزایشی باشه. پاسخگو باشه (یعنی Availability)

### Safety and liveness

دوتا از property های مهم در مورد الگوریتم ها عبارتند از Safety و liveness.

فرق هست بین Safety و liveness. مثلا در مورد الگوریتم تولید fencing token دوتا ویژگی Uniqueness و Monotonic sequence از نوع Safety هستن و ویژگی Availability از نوع liveness هست. Safety یعنی هیچ اتفاق بدی نیفته، liveness یعنی نهایتا یه اتفاق خوب میفته.

وقتی یکی از ویژگی های از جنس safety نقض میشه، میشه نمونه از اون نقض شدگی رو در هیزتوری سیستم پیدا کرد. و اینکه وقتی این ویژگی نقض شد دیگه نقض شده. نمیشه امیدوارم بود که اوضاع بهتر بشه.
اما وقتی یکی از ویژگی های از جنس liveness اینطور نیستن. همیشه امید هست که شرایط در اینده رضایت بخش بشه.

تفکیک معنی safety و liveness بهمون کمک میکنه که مدل سیستم ها رو بهتر بتونیم بشناسیم. در مورد الگوریتم های توزیع شده معمول این هست که ویژگی safetyتحت هر شرایطی باید برقرار باشه. حتی اگر همه نودها crash کردند و نتوورک کلا قطع بشه الگوریتم باید درست کار بکنه و خروجی درست باشه.

اما در مورد ویزگی liveness این هشدار پذیرفته هست که به شرطی هر ریکوئستی پاسخ دریافت خواهد کرد که اپثریت نودها crash نکرده باشن. و شبکه حتی اگر اختلالی داشت، بتواند نهایتا برقرار شود.

### Mapping system models to the real world

به کمک ویژگی های Safety و liveness و همچنین به کمک مدلهای سیستم هست که میشه راجع به صحت الگوریتم توزیع شده اظهار نظر کرد.مدل سیستم در واقع یه تصویر اتزاعی از واقعیت هست برای همین موقع پیاده سازی الگوریتم پیچیدگی های محیط واقعی کار ما رو سخت میکنن.

مثلا الگوریتم ها در یک مدل که قراره خاصیت crash-recovery داشته باشه معمولا فرض اش اینه که داده ها در یک استوریج استیبل در امان هستند. اما اگر در سطح سخت افزار یا firmware یه باگی باشه یه خطایی یا یه misconfiguration ای باشه و باعث بشه دیتا ازبین بره چی میشه؟

الگوریتم های Quorum اتکاشون به نودی هست که در عین حال که ادعا میکنه دیتا رو ذخیره کرده همچنان ادعاش صادقه. اگر اینطور نباشه الگوریتم دیگه صحت نداره. شاید لازمه یه مدلی سیستمی دیگه ای به وجود بیاد که بتونه با احتمال lost شده دیتا کنار بیاد.  

اینجا داره توصیف نئوری از یک الگوریتم رو در مقابل عملکرد تجربی آن الگوریتم مقایسه میکنه. 
اینکه در نگاه نئوری میشه به سادگی فرض کرد که خیلی از اتفاق های بد قرار نیست بیفته. میشه فرض کرد که سیستم از نوع non-Byzantine هست. اما در واقعیت خیلی از این اتفاق هایی که گذاشتیم کنار اتفاق میفتن و ما رو غافل گیر میکنن.
این به این معنی نیست که مدل انتزاعی و نئوری از سیستم فاقد ارزش هست. بلکه خیلی کمک میکند به اینکه پیچیدگی های یک سیستم واقعی ساده سازی بشه و بشه بهتر مساله رو فهمید و براش راه حل داد.
بلکه کتاب میخاد اینو بگه که کاملا این امکان وجود داره که یه سیستم که پیاده سازی و تست شده در محیط واقعی درست کار نکنه. و این به معنی این نیست که تست در محیط نيوری فایده نداره بلکه مرحله خیلی کمک کننده ای هست. چون توسعه در محیط تئوری باعث میشه خیلی از مشکلات محیط واقعی تو هیمن مرحله کشف میشه.
